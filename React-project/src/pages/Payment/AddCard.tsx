...
const AddCard: FC = () => {
    ...
    const handleCardHolderName = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const handleExpiryDateSet = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    ...
    <div className="flex justify-start items-center mt-6 gap-[6rem] px-6">
      <Link className="w-10 h-10 bg-white border-2 border-gray-200 rounded-full flex justify-center items-center" to={"/pay/paymentMethod"}>
        <img src="/black-arrow.png" className="w-5 h-3"></img>
      </Link>
      <p className="font-semibold ff-Inter text-gray-600">Add Card</p>
    </div>
    <div className="mt-6 bg-blue-500 w-80 h-48 mx-8 flex flex-col rounded-xl p-5 relative overflow-hidden">
        <div className="flex justify-end items-start w-full"><img src="/visa.png" className="w-11 h-4"></img></div>
        <div className="flex justify-start items-center mt-16 pl-3">
            <input value={cardNumber} className="bg-blue-500 text-white placeholder-white font-medium ff-Inter w-[14rem] text-xl placeholder:text-2xl" placeholder="**** **** **** ****"/>
        </div>
        <div className="flex">
            <div className="pl-3 mt-3">
                <p className="text-6 ff-Inter text-white mb-[-2.5%] w-[8rem]">Card holder name</p>
                <input className="bg-blue-500 text-sm text-white font-semibold w-[8rem] placeholder:text-white" placeholder="Esther Howard" value={cardHolderName}></input>
            </div>
            <div className="pl-3 mt-3">
                <p className="text-6 ff-Inter text-white mb-[-2.5%] w-[6rem]">Expiry date</p>
                <input className="bg-blue-500 text-sm text-white font-semibold w-[6rem] placeholder:text-white" placeholder="02/30" value={expiryDate}></input>
            </div>
            <img src="/sim.svg" className="w-12 h-7 mt-4"></img>
        </div>
        <div id="circle1" className="absolute w-60 h-60 bg-white bg-opacity-25 rounded-full left-[60%] top-[-65%]"></div>
        <div id="circle2" className="absolute w-60 h-60 bg-white bg-opacity-25 rounded-full left-[30%] top-[-65%]"></div>
    </div>
    <form className="mt-7 px-6">
        <TextField label="Card Holder Name" placeholder="Enter your name" onChange={handleCardHolderName} type={"text"} />
        <TextField label="Card Number" placeholder="5555 5555 5555 5555" onChange={handleCardNumberChange} type={"tel"} />
        <div className="flex items-center">
          <TextField label="Expiry Date" placeholder="01/01" onChange={handleExpiryDateSet} type={"text"} style={{width: '9.6rem'}}/>
          <TextField label="CVV" placeholder="000" type={""} style={{width: '9.6rem'}}/>
        </div>
        <div className="flex items-center pl-3 mt-1">
          <input id="default-checkbox" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2"/>
          <label className="ms-2 text-sm font-medium text-gray-600 ff-Inter">Save Card</label>
        </div>
    </form>
    <div className="rounded-tl-2xl rounded-tr-2xl w-96 h-28 border-t-2 flex flex-col justify-center z-50 bg-white bg-opacity-100 absolute top-[97%] px-6" style={{ boxShadow: "0px -5px 15px rgba(0, 0, 0, 0.1)" }}>
        <Link to={"/pay/reviewSummary"}><Button Text={"Add Card"} className="h-12 mt-2 w-full"></Button></Link>
